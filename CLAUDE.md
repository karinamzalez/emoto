...
- **Mapping pipeline composition rule**: `AudioMaterialPipeline.tick(features, dtMs, lfoDeltas?)` merges LFO deltas into the raw audio target _before_ per-prop easing: `finalValue = baseDefault + audioDelta + lfoDelta`, all smoothed together by `onePole`. Pass LFO contributions for props declared in `DropletAudioProps` (e.g. `thickness`, `iridescenceIOR`) as `lfoDeltas`; apply post-easing addition only for props outside that type (e.g. `roughness` is added manually as `BASE_ROUGHNESS + lfo.roughness` after `tick()` returns).
